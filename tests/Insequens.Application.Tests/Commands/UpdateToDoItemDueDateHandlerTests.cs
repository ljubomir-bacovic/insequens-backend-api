...
public class UpdateToDoItemDueDateHandlerTests
{
    ...
    public async Task Handle_WithOwnedItem_UpdatesDueDateAndSavesChanges(DateOnly dueDate)
    {
        var userId = Guid.NewGuid();
        var itemId = Guid.NewGuid();
        var request = new UpdateToDoItemDueDateCommand(itemId, userId, dueDate);
        var item = new ToDoItemEntity { Id = itemId, UserId = userId, DueDate = null };
        var repository = Substitute.For<IRepository<ToDoItemEntity>>();
        var dataContext = Substitute.For<IDataContext>();
        var handler = new UpdateToDoItemDueDateHandler(dataContext);
    ...
    public async Task Send_WithNonexistentItem_ThrowsToDoItemNotFoundExceptionBeforeInvokingHandler()
    {
        var userId = Guid.NewGuid();
        var itemId = Guid.NewGuid();
        var request = new UpdateToDoItemDueDateCommand(itemId, userId, new DateOnly(2026, 1, 1));
        var repository = Substitute.For<IRepository<ToDoItemEntity>>();
        var dataContext = Substitute.For<IDataContext>();
